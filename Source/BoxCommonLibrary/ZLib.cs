...
namespace TheBox.Common
{
	...
	public class BoxZLib
	{
		...
		public static byte[] Compress(object source)
		{
			...
			{
				Clipboard.SetDataObject(err.ToString());
				return new byte[0];
			}
		}
		...
		public static byte[] Decompress(byte[] data)
		{
			...
			var realData = new byte[data.Length - 4];
			Array.Copy(data, 4, realData, 0, data.Length - 4);

			var result = new byte[length];

			var err = uncompress(result, ref length, realData, realData.Length);
